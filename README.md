...
- Модуль **utils**, в котором реализованы функции:
  - get_transaction_list: читает список транзакций из указанного файла,
  в случае каких-либо ошибок возвращает пустой список
  - transaction_amount: считает общую сумму транзакций в рублях, если сумма была указана в другой валюте,
  производит конвертацию по текущему курсу с помощью внешнего API
- Модуль **external_api**, в котором реализованы функции:
  - external_api_exchange: функция конвертации валют в рубли с помощью внешнего API.
  Для корректной работы не забудьте указать ваш токен в файле .env. Пример заполнения файла см. в .env.example
...
- Модуль decorators, тесты test_log_decorator_console, test_log_decorator_exception_console, test_log_decorator_txt,
test_log_decorator_exception_txt:
  
  - Проверка записи лога в указанный файл в случае нормальной работы функции и в случае возникновения ошибок
  - Проверка вывода лога в консоль в случае нормальной работы функции и в случае возникновения ошибок
  - Ввиду наличие динамических данных проверка осуществляется путем поиска совпадающих слов для каждого вида лога
  - Описание тестов см. в документации к юнит-тестам

- Модуль utils, тесты test_get_transaction_list и test_transaction_amount.
Описание каждого теста см. в документации к юнит-тестам
- Модуль external_api, тест test_external_api_exchange.
Описание см. в документации к юнит-тестам
